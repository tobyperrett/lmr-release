...
class Epickitchens(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode):
        ...
        print("verb/noun counts: ", self.get_class_counts())
    ...
    def _construct_loader(self):
        ...
        # self.action_labels = [r.label for r in self._video_records]
        # print(self.action_labels)
        self.verb_labels = [r.label['verb'] for r in self._video_records]
        self.verb_label_dict = {}
        for vc in range(max(self.verb_labels)):
            indexes = [index for (index, item) in enumerate(self.verb_labels) if item == vc]
            if len(indexes) > 0:
                self.verb_label_dict[vc] = indexes

        self.noun_labels = [r.label['noun'] for r in self._video_records]
        self.noun_label_dict = {}
        for vc in range(max(self.noun_labels)):
            indexes = [index for (index, item) in enumerate(self.noun_labels) if item == vc]
            if len(indexes) > 0:
                self.noun_label_dict[vc] = indexes        
        ...
        if self.mode in ["train", "val", "train+val"]:
            ...
            if self.class_balancing:
                if self.cfg.EPICKITCHENS.LT_TASK == "verb":
                    verb_class, verb_class_indexes = random.choice(list(self.verb_label_dict.items()))
                    index = random.choice(verb_class_indexes)
                elif self.cfg.EPICKITCHENS.LT_TASK == "noun":
                    noun_class, noun_class_indexes = random.choice(list(self.noun_label_dict.items()))
                    index = random.choice(noun_class_indexes)
                else:
    ...
    def get_class_counts(self):
        ...
        for index in range(len(self)):
            ...
            if n in noun_counts:
                noun_counts[n] = noun_counts[n] + 1
            else:
                noun_counts[n] = 1

        verb_counts_all = [0] * 97
        for i in range(len(verb_counts_all)):
            if i in verb_counts.keys():
                verb_counts_all[i] = verb_counts[i]

        noun_counts_all = [0] * 300
        for i in range(len(noun_counts_all)):
            if i in noun_counts.keys():
                noun_counts_all[i] = noun_counts[i]

        return verb_counts_all, noun_counts_all