...
def lt_epoch_stats(correct, counts=None, dist_splits=None):

    # print(correct)
    # print(counts)
    # print(dist_splits)
    
    class_accs = correct / (counts + 0.01)

    avg_accs = []

    avg_accs.append(np.mean(class_accs))

    for split in dist_splits:
        split_accs = []
        for i in split:
            split_accs.append(class_accs[i])
        if len(split_accs) > 0:
            split_accs = np.mean(np.stack(split_accs))
        else:
            split_accs = -1
        avg_accs.append(split_accs)
    avg_accs = np.array(avg_accs)
    avg_accs = [float(a) for a in avg_accs]
...
class TestMeter(object):
    ...
    def update_stats(self, preds, labels, clip_ids, metadata):
        ...
        for ind in range(preds.shape[0]):
            ...
            self.metadata[vid_id] = metadata[ind]
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            lt, lt_cor, lt_count, class_accs, class_counts = metrics.lt_accuracies(self.video_preds, self.video_labels, self.cfg.SSV2.TRAIN_COUNTS, self.cfg.SSV2.DIST_SPLITS)
            # print(class_accs)
            # print(class_counts)
            self.stats["lt"] = lt.detach().cpu().numpy().tolist()
            self.stats["class_accs"] = class_accs
...
class ScalarMeter(object):
    ...
    def add_value(self, value):
        ...
        #         self.total = value
        #     else:
        #         self.total = [self.total[i] + value[i] for i in range(len(self.total))]
        # else:
        self.total += value
    ...
    def get_win_median(self):
        ...
        """
        # if isinstance(self.deque[0], list):
        if isinstance(self.deque[0], np.ndarray):
            all_q = [e for e in self.deque]
            avgs = []
            for i in range(len(all_q[0])):
                avg = float(np.median([q[i] for q in all_q]))
                avgs.append(avg)
            return avgs
    ...
    def get_win_avg(self):
        ...
        if isinstance(self.deque[0], np.ndarray):
            ...
            avgs = []
            for i in range(len(all_q[0])):
                avg = float(np.mean([q[i] for q in all_q]))
                avgs.append(avg)
            return avgs
    ...
    def get_global_avg(self):
        ...
        if isinstance(self.total, np.ndarray):
            return [t / self.count for t in self.total]
...
class TrainMeter(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        # lt
        self.mb_lt = ScalarMeter(cfg.LOG_PERIOD)
        self.lt_cor = 0
        self.lt_count = 0
    ...
    def update_stats(self, top1_err, top5_err, lt_acc, loss, lr, mb_size):
        """
        Update the current stats.
        Args:
        ...
        if not self.cfg.DATA.MULTI_LABEL:
        ...
        self.mb_lt.add_value(lt_acc[0])
        if isinstance(self.lt_cor, int):
            self.lt_cor = lt_acc[1]
            self.lt_count = lt_acc[2]
        else:
            self.lt_cor = self.lt_cor + lt_acc[1]
            self.lt_count = self.lt_count + lt_acc[2]
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            "_type": "train_iter",
            "epoch": "{}/{}".format(cur_epoch + 1, self.cfg.SOLVER.MAX_EPOCH),
...
class ValMeter(object):
    ...
    def update_stats(self, top1_err, top5_err, lt_acc, mb_size):
        """
        Update the current stats.
        Args:
            top1_err (float): top1 error rate.
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if self.cfg.DATA.MULTI_LABEL:
            ...
            lt, all_class_accs = lt_epoch_stats(self.lt_cor, self.lt_count, self.cfg.SSV2.DIST_SPLITS)
            ...
            stats["lt"] = lt
            # stats["class_accs"] = all_class_accs
            # stats["lt_cor"] = lt_cor
            # stats["lt_count"] = lt_count
...
class EPICTrainMeter(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        # lt
        self.mb_lt_verb = ScalarMeter(cfg.LOG_PERIOD)
        self.mb_lt_noun = ScalarMeter(cfg.LOG_PERIOD)
        self.lt_verb_cor = 0
        self.lt_verb_count = 0
        self.lt_noun_cor = 0
        self.lt_noun_count = 0
    ...
    def reset(self):
        ...
        # lt
        self.mb_lt_verb.reset()
        self.mb_lt_noun.reset()
        self.lt_verb_cor = 0
        self.lt_verb_count = 0
        self.lt_noun_cor = 0
        self.lt_noun_count = 0
    ...
    def update_stats(self, top1_acc, top5_acc, lt_acc, loss, lr, mb_size):
        """
        Update the current stats.
        Args:
        ...
        self.mb_lt_verb.add_value(lt_acc[0])
        self.mb_lt_noun.add_value(lt_acc[1])
        if isinstance(self.lt_verb_cor, int):
            self.lt_verb_cor = lt_acc[2]
            self.lt_noun_cor = lt_acc[3]
            self.lt_verb_count = lt_acc[4]
            self.lt_noun_count = lt_acc[5]
        else:
            self.lt_verb_cor = self.lt_verb_cor + lt_acc[2]
            self.lt_noun_cor = self.lt_noun_cor + lt_acc[3]
            self.lt_verb_count = self.lt_verb_count + lt_acc[4]
            self.lt_noun_count = self.lt_noun_count + lt_acc[5]
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            "lt_verb": self.mb_lt_verb.get_win_median(),
            "lt_noun": self.mb_lt_noun.get_win_median()
        }
        logging.log_json_stats(stats)
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        lt_verb, all_verb_accs = lt_epoch_stats(self.lt_verb_cor, self.lt_verb_count, self.cfg.EPICKITCHENS.VERB_DIST_SPLITS)
        lt_noun, all_noun_accs = lt_epoch_stats(self.lt_noun_cor, self.lt_noun_count, self.cfg.EPICKITCHENS.NOUN_DIST_SPLITS)
...
class EPICValMeter(object):
    ...
    def update_stats(self, top1_acc, top5_acc, lt_acc, mb_size):
        """
        Update the current stats.
        Args:
            top1_acc (float): top1 accuracy rate.
...
class EPICTestMeter(object):
    ...
    def __init__(self, num_videos, num_clips, num_cls, overall_iters, cfg):
        """
        Construct tensors to store the predictions and labels. Expect to get
        num_clips predictions from each video, and calculate the metrics on
        num_videos videos.
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        verb_lt, verb_lt_cor, verb_lt_count, all_verb_accs, all_verb_counts = metrics.lt_accuracies(self.verb_video_preds, self.verb_video_labels, self.cfg.EPICKITCHENS.TRAIN_VERB_COUNTS, self.cfg.EPICKITCHENS.VERB_DIST_SPLITS)
        noun_lt, noun_lt_cor, noun_lt_count, all_noun_accs, all_noun_counts= metrics.lt_accuracies(self.noun_video_preds, self.noun_video_labels, self.cfg.EPICKITCHENS.TRAIN_NOUN_COUNTS, self.cfg.EPICKITCHENS.NOUN_DIST_SPLITS)
        ...
        stats["verb_lt"] = verb_lt.detach().cpu().numpy().tolist()
        stats["noun_lt"] = noun_lt.detach().cpu().numpy().tolist()

        stats["verb_accs"] = all_verb_accs
        stats["verb_counts"] = all_verb_counts
        stats["noun_accs"] = all_noun_accs
        stats["noun_counts"] = all_noun_counts
