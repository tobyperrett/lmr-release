...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, loss_scaler=None):
    """
    Save a checkpoint.
    Args:
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            not_load_layers_after_experts = []
            for k in not_load_layers:
                pre_train_k = k.replace('.0.', '.')
                try:
                    pre_train_dict_match[k] = pre_train_dict[pre_train_k]
                except:
                    not_load_layers_after_experts.append(k)
            not_load_layers = not_load_layers_after_experts
...
def load_train_checkpoint(cfg, model, optimizer, loss_scaler=None):
    """
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            last_checkpoint, model, cfg.NUM_GPUS > 1, optimizer, loss_scaler=loss_scaler
        )
